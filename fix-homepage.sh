...
# Script per configurare la homepage
# Uso: ./fix-homepage.sh
...
if [ -n "$HOME_ID" ] && [ "$HOME_ID" != "" ]; then
    ...
    echo ""
    echo "Per configurare manualmente, esegui:"
    echo "  mysql -uroot -proot archivio_local -e \"UPDATE wp_options SET option_value = 'page' WHERE option_name = 'show_on_front'; UPDATE wp_options SET option_value = 'ID_PAGINA' WHERE option_name = 'page_on_front';\""
fi
