...
if ( ! class_exists( 'Valeska_Rest_API' ) ) {
	/**
	 * Rest API class with configuration
	 */
	class Valeska_Rest_API {
		private static $instance;
		private $namespace;

		public function __construct() {
			// Init variables
			$this->set_namespace( 'valeska/v1' );

			// Localize theme's main js script with rest variable
			add_filter( 'valeska_filter_localize_main_js', array( $this, 'localize_script' ) );
		...
		function localize_script( $global ) {
			...
			return apply_filters( 'valeska_filter_rest_api_global_variables', $global, $this->get_namespace() );
		}

		function register_rest_api_route() {
			$routes = apply_filters( 'valeska_filter_rest_api_routes', array() );
