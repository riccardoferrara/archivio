...
if ( ! function_exists( 'valeska_add_rest_api_pagination_global_variables' ) ) {
	 ...
	 * @return array
	 */
	function valeska_add_rest_api_pagination_global_variables( $global, $namespace ) {
		$global['paginationRestRoute'] = $namespace . '/get-posts';

		return $global;
	}

	add_filter( 'valeska_filter_rest_api_global_variables', 'valeska_add_rest_api_pagination_global_variables', 10, 2 );
}

if ( ! function_exists( 'valeska_add_rest_api_pagination_route' ) ) {
	/**
	 ...
	 * @return array
	 */
	function valeska_add_rest_api_pagination_route( $routes ) {
		$routes['pagination'] = array(
			'route'    => 'get-posts',
			'methods'  => WP_REST_Server::READABLE,
			'callback' => 'valeska_get_new_posts',
			'args'     => array(
				'options' => array(
					'required'          => true,
	...
	add_filter( 'valeska_filter_rest_api_routes', 'valeska_add_rest_api_pagination_route' );
}

if ( ! function_exists( 'valeska_get_new_posts' ) ) {
	/**
	...
	function valeska_get_new_posts() {
		...
		if ( ! isset( $_GET ) || empty( $_GET ) ) {
			...
			if ( ! empty( $options ) ) {
				...
				$data = apply_filters(
					'valeska_filter_pagination_data_return',
					array(
						'html'            => $html,
				...
				valeska_get_ajax_status( 'success', esc_html__( 'Items are loaded', 'valeska' ), $data );
			} else {
				valeska_get_ajax_status( 'error', esc_html__( 'Options are invalid', 'valeska' ) );
			}
		}
	}
}

if ( ! function_exists( 'valeska_get_query_params' ) ) {
	/**
	 ...
	 * @return array
	 */
	function valeska_get_query_params( $params ) {
		...
		return apply_filters( 'valeska_filter_query_params', $args, $params );
	}
}

if ( ! function_exists( 'valeska_get_pagination_data' ) ) {
	/**
	 ...
	 * @return array
	 */
	function valeska_get_pagination_data( $plugin, $module, $shortcode, $post_type, $params ) {
		$data = array();
...
if ( ! function_exists( 'valeska_add_link_pages_after_content' ) ) {
	/**
	 * Function which add pagination for blog single and page
	 */
	function valeska_add_link_pages_after_content() {

		$args_pages = array(
			'before'      => '<div class="qodef-single-links qodef-m"><span class="qodef-m-single-links-title">' . esc_html__( 'Pages: ', 'valeska' ) . '</span>',
			'after'       => '</div>',
			'link_before' => '<span>',
			'link_after'  => '</span>',
			'pagelink'    => '0%',
	...
	add_action( 'valeska_action_after_blog_single_content', 'valeska_add_link_pages_after_content' );
	add_action( 'valeska_action_after_page_content', 'valeska_add_link_pages_after_content' );
}
