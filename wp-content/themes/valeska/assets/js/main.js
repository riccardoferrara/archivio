(function ( $ ) {
			...
			return {
				on: {
					...
					init: function () {
						...
						qodef.body.trigger(
							'valeska_trigger_swiper_is_initialized',
							[$holder, options]
						);
	...
	var qodefAppearAnimation = {
		init () {
			this.holders = $('.qodef-custom-appear-columns, .qodef-offset-image-appear-animation--yes');
					...
					( index, element ) => {
						...
						qodefCore.qodefIsInViewport.check(
							...
							() => {
								...
								if ( $holder.hasClass('qodef-custom-appear-columns') ) {
									setTimeout(
										() => {
											qodef.body.addClass( 'qodef-external-trigger' );
										}, 450
									);
								}
							}
	...
	var qodefUncoverSection = {
		...
		setHeight ( $holder ) {
			...
			const sectionHeight = $holder.outerHeight();
...
(function ( $ ) {
	...
	var qodefPagination = {
		...
		initInfiniteScroll: function ( $holder ) {
			...
			if ( ! $holder.hasClass( 'qodef--loading' ) && parseInt(scrollPosition.toFixed()) >= parseInt(holderEndPosition.toFixed()) && options.max_pages_num >= options.next_page ) {
				qodefPagination.getNewPosts( $holder );
			}
		},
