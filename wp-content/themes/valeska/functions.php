...
if ( ! class_exists( 'Valeska_Handler' ) ) {
	/**
	 * Main theme class with configuration
	 */
	class Valeska_Handler {
		...
		public function __construct() {
			...
			require_once VALESKA_ROOT_DIR . '/helpers/helper.php';
			...
			add_action( 'valeska_action_before_main_css', array( $this, 'include_plugins_styles' ) );

			// Include theme's 3rd party plugins scripts
			add_action( 'valeska_action_before_main_js', array( $this, 'include_plugins_scripts' ) );

			// Add pingback header
			add_action( 'wp_head', array( $this, 'add_pingback_header' ), 1 );

			// Include theme's skip link
			add_action( 'valeska_action_after_body_tag_open', array( $this, 'add_skip_link' ), 5 );

			// Include theme's Google fonts
			add_action( 'valeska_action_before_main_css', array( $this, 'include_google_fonts' ) );
		...
		function include_css_scripts() {
			...
			$main_css_dependency = apply_filters( 'valeska_filter_main_css_dependency', array( 'swiper' ) );

			// Hook to include additional scripts before theme's main style
			do_action( 'valeska_action_before_main_css' );

			// Enqueue theme's main style
			wp_enqueue_style( 'valeska-main', VALESKA_ASSETS_CSS_ROOT . '/main.min.css', $main_css_dependency );

			// Enqueue theme's style
			wp_enqueue_style( 'valeska-style', VALESKA_ROOT . '/style.css' );

			// Hook to include additional scripts after theme's main style
			do_action( 'valeska_action_after_main_css' );
		}

		function add_inline_style() {
			$style = apply_filters( 'valeska_filter_add_inline_style', $style = '' );

			if ( ! empty( $style ) ) {
				wp_add_inline_style( 'valeska-style', $style );
			}
		}

		function include_js_scripts() {
			// JS dependency variable
			$main_js_dependency = apply_filters( 'valeska_filter_main_js_dependency', array( 'jquery' ) );

			// Hook to include additional scripts before theme's main script
			do_action( 'valeska_action_before_main_js', $main_js_dependency );

			// Enqueue theme's main script
			wp_enqueue_script( 'valeska-main-js', VALESKA_ASSETS_JS_ROOT . '/main.min.js', $main_js_dependency, false, true );

			// Hook to include additional scripts after theme's main script
			do_action( 'valeska_action_after_main_js' );
		...
		function localize_js_scripts() {
			$global = apply_filters(
				'valeska_filter_localize_main_js',
				array(
					'adminBarHeight' => is_admin_bar_showing() ? 32 : 0,
					'iconArrowLeft'  => valeska_get_svg_icon( 'slider-arrow-left' ),
					'iconArrowRight' => valeska_get_svg_icon( 'slider-arrow-right' ),
					'iconClose'      => valeska_get_svg_icon( 'close' ),
				)
			);
		...
		function include_plugins_styles() {
			...
			wp_enqueue_style( 'swiper', VALESKA_ASSETS_ROOT . '/plugins/swiper/swiper.min.css' );
		}

		function include_plugins_scripts() {

			// JS dependency variables
			$js_3rd_party_dependency = apply_filters( 'valeska_filter_js_3rd_party_dependency', 'jquery' );

			// Enqueue 3rd party plugins script
			wp_enqueue_script( 'jquery-waitforimages', VALESKA_ASSETS_ROOT . '/plugins/waitforimages/jquery.waitforimages.js', array( $js_3rd_party_dependency ), false, true );
			wp_enqueue_script( 'swiper', VALESKA_ASSETS_ROOT . '/plugins/swiper/swiper.min.js', array( $js_3rd_party_dependency ), false, true );
		}
		...
		function add_skip_link() {
			echo '<a class="skip-link screen-reader-text" href="#qodef-page-content">' . esc_html__( 'Skip to the content', 'valeska' ) . '</a>';
		}
		...
		function set_theme_support() {
			...
			load_theme_textdomain( 'valeska', VALESKA_ROOT_DIR . '/languages' );
			...
			if ( ! isset( $content_width ) ) {
				$content_width = apply_filters( 'valeska_filter_set_content_width', 1300 );
			}
			...
			add_editor_style( VALESKA_ASSETS_CSS_ROOT . '/editor-style.min.css' );
		}
		...
		function editor_customizer_styles() {
			...
			wp_enqueue_style( 'valeska-editor-customizer-styles', VALESKA_ASSETS_CSS_ROOT . '/editor-customizer-style.css' );

			// Add Gutenberg blocks style
			wp_enqueue_style( 'valeska-gutenberg-blocks-style', VALESKA_INC_ROOT . '/gutenberg/assets/admin/css/gutenberg-blocks.css' );
		}
		...
		function add_body_classes( $classes ) {
			...
			return apply_filters( 'valeska_filter_add_body_classes', $classes );
		}

		function include_modules() {

			// Hook to include additional files before modules inclusion
			do_action( 'valeska_action_before_include_modules' );

			foreach ( glob( VALESKA_INC_ROOT_DIR . '/*/include.php' ) as $module ) {
				include_once $module; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
			}

			// Hook to include additional files after modules inclusion
			do_action( 'valeska_action_after_include_modules' );
		}
	}

	Valeska_Handler::get_instance();
}