...
if ( ! function_exists( 'valeska_core_get_google_maps_api_key' ) ) {
	/**
	 ...
	 * @return bool|string
	 */
	function valeska_core_get_google_maps_api_key( $return_type = 'option' ) {
		$option = valeska_core_get_option_value( 'admin', 'qodef_maps_api_key' );
...
if ( ! function_exists( 'valeska_core_set_multiple_map_variables' ) ) {
	/**
	 ...
	 */
	function valeska_core_set_multiple_map_variables( $query = array() ) {
		$map_variables = array();
		...
		if ( is_array( $query ) && count( $query ) ) {
			...
			if ( ! empty( $items ) ) {
				foreach ( $items as $id => $title ) {
					$map_variables['addresses'][] = valeska_core_generate_map_params( $id, $query['post_type'] );
				}
			}
		}
...
if ( ! function_exists( 'valeska_core_get_multiple_map' ) ) {
	/**
	 ...
	 * @return string
	 */
	function valeska_core_get_multiple_map( $query = array() ) {
		$addresses = valeska_core_set_multiple_map_variables( $query );
		...
		if ( valeska_core_get_google_maps_api_key( 'is_enabled' ) ) {
			...
			$html = '<p id="qodef-multiple-map-notice">' . esc_html__( 'In order for the map functionality to be enabled please input the Google Map API key in the General section of the Valeska Options', 'valeska-core' ) . '</p>';
		}

		do_action( 'valeska_core_action_after_multiple_map' );
...
if ( ! function_exists( 'valeska_core_marker_info_template' ) ) {
	/**
	 * Template with placeholders for marker info window
	 *
	 * uses underscore templates
	 */
	function valeska_core_marker_info_template() {
	...
	add_action( 'valeska_core_action_after_multiple_map', 'valeska_core_marker_info_template' );
}

if ( ! function_exists( 'valeska_core_marker_template' ) ) {
	/**
	 * Template with placeholders for marker
	 */
	function valeska_core_marker_template() {
	...
	add_action( 'valeska_core_action_after_multiple_map', 'valeska_core_marker_template' );
}
...
if ( ! function_exists( 'valeska_core_generate_map_params' ) ) {
	/**
	 ...
	 * @return array
	 */
	function valeska_core_generate_map_params( $item_id, $post_type ) {
		$map_params = array();
		...
		//get address params
		$address_array = apply_filters( 'valeska_core_filter_address_params', array(), $item_id, $post_type );
