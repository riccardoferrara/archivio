...
abstract class ValeskaCore_Header {
	...
	public function __construct() {
		...
		add_action( 'valeska_core_action_before_main_css', array( $this, 'enqueue_assets' ) );
		add_action( 'valeska_core_action_before_main_css', array( $this, 'enqueue_additional_assets' ) );
		add_filter( 'valeska_filter_localize_main_js', array( $this, 'set_global_javascript_variables' ) );
		add_filter( 'valeska_core_filter_content_margin', array( $this, 'get_content_margin' ) );
		add_filter( 'valeska_core_filter_title_padding', array( $this, 'get_title_padding' ) );
		add_filter( 'valeska_filter_add_inline_style', array( $this, 'set_inline_header_styles' ) );
		add_filter( 'valeska_filter_header_inner_class', array( $this, 'set_header_inner_classes' ), 10, 2 );
		add_filter( 'valeska_core_filter_nav_menu_header_selector', array( $this, 'set_nav_menu_header_selector' ) );
		add_filter( 'valeska_core_filter_nav_menu_narrow_header_selector', array( $this, 'set_nav_menu_narrow_header_selector' ) );
	}
	...
	public function load_template( $parameters = array() ) {
		$parameters = apply_filters( 'valeska_core_filter_header_template', $parameters );

		return valeska_core_get_template_part( 'header/layouts/' . $this->get_layout(), 'templates/' . $this->get_layout(), $this->get_layout_slug(), $parameters );
	}
	...
	public function get_header_transparency() {
		$background_color = valeska_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_background_color' );
	...
	public function content_behind_header() {
		return 'yes' === valeska_core_get_post_value_through_levels( 'qodef_content_behind_header' );
	}
	...
	function set_header_height() {
		$header_height = valeska_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_height' );
		$header_height = ! empty( $header_height ) ? intval( $header_height ) : $this->default_header_height;

		$this->header_height = apply_filters( 'valeska_core_filter_set_header_height', $header_height );
	}

	function set_header_inner_classes( $class, $layout ) {
		// Check is content in grid
		$class[] = 'yes' === valeska_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_in_grid' ) ? 'qodef-content-grid' : '';
	...
	public function set_inline_header_styles( $style ) {
		...
		$height           = valeska_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_height' );
		$background_color = valeska_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_background_color' );
		...
		$side_padding = valeska_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_side_padding' );
		$border_color = valeska_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_border_color' );
		$border_width = valeska_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_border_width' );
		$border_style = valeska_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_border_style' );
