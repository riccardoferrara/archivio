...
class ValeskaCore_Top_Area {
	...
	public function __construct() {
		...
		add_action( 'valeska_action_before_page_header', array( $this, 'load_template' ) );
		...
		add_filter( 'valeska_filter_add_inline_style', array( $this, 'set_inline_top_area_styles' ) );
		add_filter( 'valeska_core_filter_top_area_inner_class', array( $this, 'set_grid_class' ) );
	}

	/**
	 * @return ValeskaCore_Top_Area
	 */
	...
	function is_top_area_enabled() {
		$top_area_enabled = 'yes' === valeska_core_get_post_value_through_levels( 'qodef_top_area_header' );
	...
	public function set_top_area_height() {
		if ( $this->is_top_area_enabled ) {
			$height         = valeska_core_get_post_value_through_levels( 'qodef_top_area_header_height' );
			$top_bar_height = ! empty( $height ) ? $height : 44;
	...
	public function is_top_area_transparent() {
		$background_color = valeska_core_get_post_value_through_levels( 'qodef_top_area_header_background_color' );
	...
	public function load_template() {
		...
		valeska_core_template_part( 'header/top-area/', 'templates/top-area', '', $parameters );
	}
	...
	public function set_inline_top_area_styles( $style ) {
		...
		$height       = valeska_core_get_post_value_through_levels( 'qodef_top_area_header_height' );
		$side_padding = valeska_core_get_post_value_through_levels( 'qodef_top_area_header_side_padding' );
		$border_color = valeska_core_get_post_value_through_levels( 'qodef_top_area_header_border_color' );
		$border_width = valeska_core_get_post_value_through_levels( 'qodef_top_area_header_border_width' );
		$border_style = valeska_core_get_post_value_through_levels( 'qodef_top_area_header_border_style' );
	...
	public function set_grid_class( $classes ) {
		if ( 'yes' === valeska_core_get_post_value_through_levels( 'qodef_top_area_header_in_grid' ) ) {
...
ValeskaCore_Top_Area::get_instance();
