...
class ValeskaCore_Headers {
	...
	public function __construct() {
		...
		add_action( 'valeska_action_after_page_header_inner', array( $this, 'scroll_appearance' ) );
	...
	function include_elements() {
		...
		foreach ( glob( VALESKA_CORE_INC_PATH . '/header/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}

		foreach ( glob( VALESKA_CORE_INC_PATH . '/header/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}

		foreach ( glob( VALESKA_CORE_INC_PATH . '/header/*/include.php' ) as $header_part ) {
			include_once $header_part;
		}
	}

	function set_variables() {
		$layout_meta = valeska_core_get_post_value_through_levels( 'qodef_header_layout' );
		$layouts     = apply_filters( 'valeska_core_filter_register_header_layouts', $header_layouts_option = array() );
	...
	function render_template() {
		...
		if ( ! empty( $header_object ) ) {
			$template_hook = $header_object->is_whole_header_override() ? 'valeska_filter_header_template' : 'valeska_filter_header_content_template';
	...
	function add_body_classes( $classes ) {
		...
		$header_layout            = valeska_core_get_post_value_through_levels( 'qodef_header_layout' );
	...
	function scroll_appearance() {
		...
		if ( ! empty( $header_object ) ) {
			$appearance_type = valeska_core_get_post_value_through_levels( 'qodef_header_scroll_appearance' );

			if ( file_exists( VALESKA_CORE_HEADER_LAYOUTS_PATH . '/' . $header_object->get_layout() . '/templates/' . $appearance_type . '.php' ) ) {
				$scroll_appearance_layout = 'layouts/' . $header_object->get_layout();
			} else {
				$scroll_appearance_layout = 'scroll-appearance/' . $appearance_type;
			}

			valeska_core_template_part( 'header/' . $scroll_appearance_layout, 'templates/' . $appearance_type, '', array() );
		}
	}
	...
	function add_header_widget_areas() {
		register_sidebar(
			array(
				...
				'name'          => esc_html__( 'Header - Area One', 'valeska-core' ),
				'description'   => esc_html__( 'Widgets added here will appear in header widget area one', 'valeska-core' ),
		...
		register_sidebar(
			array(
				...
				'name'          => esc_html__( 'Header - Area Two', 'valeska-core' ),
				'description'   => esc_html__( 'Widgets added here will appear in header widget area two', 'valeska-core' ),
		...
		do_action( 'valeska_core_action_additional_header_widgets_area' );
	}
}

ValeskaCore_Headers::get_instance();
