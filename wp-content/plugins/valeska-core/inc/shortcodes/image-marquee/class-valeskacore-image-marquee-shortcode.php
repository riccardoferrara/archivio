...
if ( ! function_exists( 'valeska_core_add_image_marquee_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function valeska_core_add_image_marquee_shortcode( $shortcodes ) {
		$shortcodes[] = 'ValeskaCore_Image_Marquee_Shortcode';

		return $shortcodes;
	}

	add_filter( 'valeska_core_filter_register_shortcodes', 'valeska_core_add_image_marquee_shortcode' );
}

if ( class_exists( 'ValeskaCore_Shortcode' ) ) {
	class ValeskaCore_Image_Marquee_Shortcode extends ValeskaCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'valeska_core_filter_image_marquee_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'valeska_core_filter_image_marquee_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( VALESKA_CORE_SHORTCODES_URL_PATH . '/image-marquee' );
			$this->set_base( 'valeska_core_image_marquee' );
			$this->set_name( esc_html__( 'Image Marquee', 'valeska-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds Image Marquee element', 'valeska-core' ) );
			...
			$this->set_option(
				array(
					'field_type'    => 'text',
					'name'          => 'duration',
					'title'         => esc_html__( 'Animation Duration (Seconds)', 'valeska-core' ),
					'default_value' => '20',
				)
			);
			$this->set_option(
		...
		public function render( $options, $content = null ) {
			...
			return valeska_core_get_template_part( 'shortcodes/image-marquee', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
