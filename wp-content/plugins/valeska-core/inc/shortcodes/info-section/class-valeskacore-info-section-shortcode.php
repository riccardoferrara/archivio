...
if ( ! function_exists( 'valeska_core_add_info_section_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function valeska_core_add_info_section_shortcode( $shortcodes ) {
		$shortcodes[] = 'ValeskaCore_Info_Section_Shortcode';

		return $shortcodes;
	}

	add_filter( 'valeska_core_filter_register_shortcodes', 'valeska_core_add_info_section_shortcode' );
}

if ( class_exists( 'ValeskaCore_Shortcode' ) ) {
	class ValeskaCore_Info_Section_Shortcode extends ValeskaCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'valeska_core_filter_info_section_layouts', array() ) );

			$options_map   = valeska_core_get_variations_options_map( $this->get_layouts() );
			$default_value = $options_map['default_value'];

			$this->set_extra_options( apply_filters( 'valeska_core_filter_info_section_extra_options', array(), $default_value ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( VALESKA_CORE_SHORTCODES_URL_PATH . '/info-section' );
			$this->set_base( 'valeska_core_info_section' );
			$this->set_name( esc_html__( 'Info Section', 'valeska-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds info section element', 'valeska-core' ) );
		...
		public function render( $options, $content = null ) {
			...
			$atts = apply_filters( 'valeska_core_filter_info_section_variation_atts', $atts );

			return valeska_core_get_template_part( 'shortcodes/info-section', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes   = apply_filters( 'valeska_core_filter_info_section_variation_classes', $holder_classes, $atts );
