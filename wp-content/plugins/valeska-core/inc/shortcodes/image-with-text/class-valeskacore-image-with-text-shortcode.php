...
if ( ! function_exists( 'valeska_core_add_image_with_text_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function valeska_core_add_image_with_text_shortcode( $shortcodes ) {
		$shortcodes[] = 'ValeskaCore_Image_With_Text_Shortcode';

		return $shortcodes;
	}

	add_filter( 'valeska_core_filter_register_shortcodes', 'valeska_core_add_image_with_text_shortcode' );
}

if ( class_exists( 'ValeskaCore_Shortcode' ) ) {
	class ValeskaCore_Image_With_Text_Shortcode extends ValeskaCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'valeska_core_filter_image_with_text_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'valeska_core_filter_image_with_text_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( VALESKA_CORE_SHORTCODES_URL_PATH . '/image-with-text' );
			$this->set_base( 'valeska_core_image_with_text' );
			$this->set_name( esc_html__( 'Image With Text', 'valeska-core' ) );
			...
			$this->set_option(
				array(
					...
					'options'    => array(
						...
						'scrolling-image' => esc_html__( 'Scrolling Image', 'valeska-core' ),
					),
				)
			);
            $this->set_option(
                array(
                    'field_type'  => 'select',
                    'name'        => 'scrolling_image_type',
                    'title'       => esc_html__( 'Scrolling Image Type', 'valeska-core' ),
                    'options'     => array(
                        'type-1' => esc_html__( 'Type 1', 'valeska-core' ),
                        'type-2' => esc_html__( 'Type 2', 'valeska-core' ),
                    ),
                    'dependency' => array(
                        'show' => array(
                            'image_action' => array(
            ...
            $this->set_option(
                array(
                    ...
                    'title'       => esc_html__( 'Scrolling Direction', 'valeska-core' ),
                    'options'     => array(
			...
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Enable Box Shadow on Image', 'valeska-core' ),
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'valeska_core_image_with_text', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return valeska_core_get_template_part( 'shortcodes/image-with-text', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
		...
		private function get_holder_classes( $atts ) {
            ...
            $holder_classes[] = ( 'yes' === $atts['enable_box_shadow'] ) ? 'qodef--has-shadow' : '';
            $holder_classes[] = ! empty( $atts['appear_animation'] ) ? 'qodef-iwt-appear-animation--' . $atts['appear_animation'] : '';
