...
if ( ! function_exists( 'valeska_core_add_icon_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function valeska_core_add_icon_shortcode( $shortcodes ) {
		$shortcodes[] = 'ValeskaCore_Icon_Shortcode';

		return $shortcodes;
	}

	add_filter( 'valeska_core_filter_register_shortcodes', 'valeska_core_add_icon_shortcode', 8 );
}

if ( class_exists( 'ValeskaCore_Shortcode' ) ) {
	class ValeskaCore_Icon_Shortcode extends ValeskaCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( VALESKA_CORE_SHORTCODES_URL_PATH . '/icon' );
			$this->set_base( 'valeska_core_icon' );
			$this->set_name( esc_html__( 'Icon', 'valeska-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon element', 'valeska-core' ) );
			...
			$this->set_option(
				array(
					'field_type' => 'iconpack',
					'name'       => 'main_icon',
					'title'      => esc_html__( 'Icon', 'valeska-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'options'    => array(
						...
						'tiny'    => esc_html__( 'Tiny', 'valeska-core' ),
						'small'   => esc_html__( 'Small', 'valeska-core' ),
						'medium'  => esc_html__( 'Medium', 'valeska-core' ),
						'large'   => esc_html__( 'Large', 'valeska-core' ),
						'huge'    => esc_html__( 'Huge', 'valeska-core' ),
					),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_size',
					'title'      => esc_html__( 'Custom Size', 'valeska-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'options'       => array(
						'normal' => esc_html__( 'Normal', 'valeska-core' ),
						'circle' => esc_html__( 'Circle', 'valeska-core' ),
						'square' => esc_html__( 'Square', 'valeska-core' ),
					),
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'shape_size',
					'title'      => esc_html__( 'Shape Size', 'valeska-core' ),
					'group'      => esc_html__( 'Style', 'valeska-core' ),
					'dependency' => array(
						'show' => array(
							'icon_layout' => array(
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Vertical Offset', 'valeska-core' ),
					'group'      => esc_html__( 'Style', 'valeska-core' ),
				)
			);
			$this->set_option(
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'valeska_core_icon', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return valeska_core_get_template_part( 'shortcodes/icon', 'templates/icon', '', $atts );
		}
