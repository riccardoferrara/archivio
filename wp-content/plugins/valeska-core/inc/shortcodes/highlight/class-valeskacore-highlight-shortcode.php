...
if ( ! function_exists( 'valeska_core_add_highlight_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function valeska_core_add_highlight_shortcode( $shortcodes ) {
		$shortcodes[] = 'ValeskaCore_Highlight_Shortcode';

		return $shortcodes;
	}

	add_filter( 'valeska_core_filter_register_shortcodes', 'valeska_core_add_highlight_shortcode' );
}

if ( class_exists( 'ValeskaCore_Shortcode' ) ) {
	class ValeskaCore_Highlight_Shortcode extends ValeskaCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( VALESKA_CORE_SHORTCODES_URL_PATH . '/highlight' );
			$this->set_base( 'valeska_core_highlight' );
			$this->set_name( esc_html__( 'Highlight', 'valeska-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays highlight with provided parameters', 'valeska-core' ) );
			...
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Disable Text Line Break', 'valeska-core' ),
					'description'   => esc_html__( 'Enabling this option will disable text line breaks for screen size 1024 and lower', 'valeska-core' ),
			...
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Highlight Text Position', 'valeska-core' ),
					'description' => esc_html__( 'Enter the positions of the words you would like to display as "highlight" text. Separate start and end positions with commas (e.g. if you would like to wrap from fifth to ninth words, you would enter "5,9") or if you want to highlight whole text fill -1', 'valeska-core' ),
					'group'       => esc_html__( 'Text Style', 'valeska-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Highlight Text Color', 'valeska-core' ),
					'group'      => esc_html__( 'Text Style', 'valeska-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Highlight Text Background Color', 'valeska-core' ),
					'group'      => esc_html__( 'Text Style', 'valeska-core' ),
				)
			);
			$this->set_option(
		...
		public function render( $options, $content = null ) {
			...
			return valeska_core_get_template_part( 'shortcodes/highlight', 'templates/highlight', '', $atts );
		}
