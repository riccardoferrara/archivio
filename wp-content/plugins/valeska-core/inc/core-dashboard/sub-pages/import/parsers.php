...
class WXR_Parser {
	function parse( $file ) {
		...
		if ( isset($result) && defined('IMPORT_DEBUG') && IMPORT_DEBUG ) {
			...
			echo '</pre>';
			echo '<p><strong>' . esc_html__( 'There was an error when reading this WXR file', 'valeska-core' ) . '</strong><br />';
			echo esc_html__( 'Details are shown above. The importer will now try again with a different parser...', 'valeska-core' ) . '</p>';
		}
...
class WXR_Parser_SimpleXML {
	function parse( $file ) {
		...
		$url      = defined( 'VALESKA_CORE_PROFILE_SLUG' ) ? 'http://export.' . VALESKA_CORE_PROFILE_SLUG . '-themes.com/'.$file : '';
		...
		if ( ! $success || isset( $dom->doctype ) ) {
			return new WP_Error( 'SimpleXML_parse_error', esc_html__( 'There was an error when reading this WXR file', 'valeska-core' ), libxml_get_errors() );
		}
		...
		if ( ! $wxr_version )
			return new WP_Error( 'WXR_parse_error', esc_html__( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'valeska-core' ) );
