...
class ValeskaCore_Dashboard_Import {
	/**
	 * @var ValeskaCore_Dashboard_Import of current class
	 */
	...
	/**
	 * @return ValeskaCore_Dashboard_Import
	 */
	...
	public function set_import_url() {
		$params = ValeskaCore_Dashboard::get_instance()->get_import_params();
				...
				switch ( $_POST['options']['action'] ) :
					...
					case 'rev-slider':
						if ( qode_framework_is_installed( 'revolution-slider' ) ) {
							...
							$this->set_message( esc_html__( 'Revolution Slider isn\'t installed', 'valeska-core' ) );
						}
						break;
					case 'content':
						...
						if ( $content_start ) {
							if ( ! ValeskaCore_Dashboard::get_instance()->check_purchase_code( $_POST['options']['demo'] ) ) {
								qode_framework_get_ajax_status( 'error', esc_html__( 'Please don\'t try to hack me. Purchase code registered is not valid', 'valeska-core' ) );
								exit;
							}
						}
	...
	function file_content( $path ) {
		...
		if ( 200 !== intval( $response_code ) ) {
			$this->set_message( $response['response']['message'] . ' ' . esc_html__( 'Please contact support', 'valeska-core' ) );
	...
	public function import_widgets( $demo ) {
		...
		if ( $widgets_content ) {
			foreach ( (array) $widgets_content['widgets'] as $valeska_widget_id => $valeska_widget_data ) {
				update_option( 'widget_' . $valeska_widget_id, $valeska_widget_data );
			}

			$ws = $this->import_sidebars_widgets( $widgets );

			if ( $ws ) {
				$this->set_message( esc_html__( 'Widgets are set for proper sidebar', 'valeska-core' ) );
				$this->set_data( 'type', 'options' );
				$this->set_status( 'success' );
			}
		}
	...
	public function import_sidebars_widgets( $file ) {
		$valeska_sidebars = get_option( 'sidebars_widgets' );
		unset( $valeska_sidebars['array_version'] );
		$data = $this->unserialized_content( $file );

		if ( $data && is_array( $data['sidebars'] ) ) {
			$valeska_sidebars = array_merge( (array) $valeska_sidebars, (array) $data['sidebars'] );
			unset( $valeska_sidebars['wp_inactive_widgets'] );
			$valeska_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $valeska_sidebars );
			$valeska_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $valeska_sidebars );
	...
	public function import_options( $file ) {
		...
		$current_options = get_option( VALESKA_CORE_OPTIONS_NAME );

		if ( $options ) {
			if ( $current_options != $options ) {
				$result = update_option( VALESKA_CORE_OPTIONS_NAME, $options );

				if ( $result ) {
					$this->update_options_after_import( $file );
					$this->set_status( 'success' );
					$this->set_data( 'type', 'options' );
					$this->set_message( esc_html__( 'Options imported successfully', 'valeska-core' ) );

					$this->update_options_after_import( $file );

				} else {
					$this->set_status( 'error' );
					$this->set_message( esc_html__( 'Problem occurred during options import', 'valeska-core' ) );
				}
			} else {
				$this->set_status( 'success' );
				$this->set_data( 'type', 'options' );
				$this->set_message( esc_html__( 'Options are already imported', 'valeska-core' ) );
			}
		}
	}
	...
	public function import_settings_pages( $file ) {
		...
		$new_ids       = get_transient( '_valeska_core_imported_posts' );
		$fields_status = true;
		...
		if ( $pages ) {
			...
			if ( ! $fields_status ) {
				$this->set_status( 'error' );
				$this->set_message( esc_html__( 'Problem occurred during settings pages import', 'valeska-core' ) );
			} else {
				$this->set_status( 'success' );
				$this->set_data( 'type', 'options' );
				$this->set_message( esc_html__( 'Settings pages imported successfully', 'valeska-core' ) );
			}
		} else {
			$this->set_status( 'error' );
			$this->set_message( esc_html__( 'File doesn\'t exist', 'valeska-core' ) );
		}
	}
	...
	public function import_menu_settings( $file ) {
		...
		if ( $menus_data ) {
			...
			$this->set_status( 'success' );
			$this->set_data( 'type', 'options' );
			$this->set_message( esc_html__( 'Menus set for proper locations', 'valeska-core' ) );
		} else {
			$this->set_status( 'error' );
			$this->set_message( esc_html__( 'Problem during menus location set', 'valeska-core' ) );
		}
	}

	public function import_content( $file, $xml, $attachments, $post_id ) {
		ob_start();
		require_once( VALESKA_CORE_INC_PATH . '/core-dashboard/sub-pages/import/wordpress-importer.php' );
		...
		$valeska_import = new WP_Import();
		set_time_limit( 0 );

		$valeska_import->fetch_attachments = $attachments;
		$returned_value                     = $valeska_import->import( $file . $xml );

		if ( is_wp_error( $returned_value ) ) {
			$this->set_status( 'error' );
			$this->set_data( 'type', 'content' );
			$this->set_data( 'xml', $xml );
			$this->set_message( esc_html__( 'An error occurred during content import', 'valeska-core' ) );
		} else {
			$this->set_status( 'success' );
			$this->set_data( 'type', 'content' );
			$this->set_data( 'posts', $this->imported_posts );
			$this->set_message( esc_html__( 'File imported successfully', 'valeska-core' ) . ' ' . $xml );
		}
		...
		if ( $files_created ) {
			...
			if ( class_exists( 'RevSliderSliderImport' ) ) {
				...
				if ( ! empty( $import_instance ) && ! is_wp_error( $import_instance ) ) {
					...
					foreach ( $demo_rev_sliders as $demo_rev_slider ) {
						...
						if ( ! in_array( str_replace( '.zip', '', $demo_rev_slider ), $rev_sliders, true ) ) {
							...
							if ( ! $rev_slider_result['success'] ) {
								$this->set_status( 'error' );
								$this->set_message( esc_html__( 'Error while importing rev sliders', 'valeska-core' ) );
								exit;
							}
						}
					}
					...
					$this->set_status( 'success' );
					$this->set_data( 'type', 'options' );
					$this->set_message( esc_html__( 'Rev sliders imported successfully', 'valeska-core' ) );
				} else {
					$this->set_status( 'error' );
					$this->set_data( 'type', 'options' );
					$this->set_message( esc_html__( 'RevSliderSliderImport instance is empty', 'valeska-core' ) );
				}
			} else {
				$this->set_status( 'error' );
				$this->set_data( 'type', 'options' );
				$this->set_message( esc_html__( 'RevSliderSliderImport class doesn\'t exist', 'valeska-core' ) );
			}
		} else {
			$this->set_status( 'error' );
			$this->set_data( 'type', 'options' );
			$this->set_message( esc_html__( 'Files don\'t exist', 'valeska-core' ) );
		}
	}
	...
	function update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( VALESKA_CORE_OPTIONS_NAME );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( VALESKA_CORE_OPTIONS_NAME, $new_global_values );
		}
	}
	...
	function get_noimage_url( $origin_img_url ) {
		...
		return VALESKA_CORE_ASSETS_URL_PATH . '/img/' . $noimage_fname;
	}
	...
	public function populate_single_pages() {
		...
		if ( isset( $_POST ) && ! empty( $_POST ) && ! empty( $_POST['options']['demo'] ) ) {
			if ( wp_verify_nonce( $_POST['options']['nonce'], 'qodef_cd_import_nonce' ) ) {
				...
				$html = valeska_core_get_template_part( 'core-dashboard/sub-pages/import/templates', 'pages-list', '', array( 'pages' => $pages ) );

				if ( $pages ) {
					qode_framework_get_ajax_status( 'success', '', $html );
				} else {
					qode_framework_get_ajax_status( 'error', esc_html__( 'Pages don\'t exist', 'valeska-core' ), '' );
				}
			}
		}
	...
	public function is_ready_to_import() {
		$info = ValeskaCore_Dashboard_System_Info_Page::get_instance()->get_system_info();
...
ValeskaCore_Dashboard_Import::get_instance();
