...
if ( ! function_exists( 'valeska_core_add_page_sidebar_options' ) ) {
	/**
	 * Function that add general options for this module
	 */
	function valeska_core_add_page_sidebar_options() {
		...
		$page = $qode_framework->add_options_page(
			array(
				...
				'slug'        => 'sidebar',
				'icon'        => 'fa fa-book',
				'title'       => esc_html__( 'Sidebar', 'valeska-core' ),
				'description' => esc_html__( 'Global Sidebar Options', 'valeska-core' ),
			)
		);
		...
		if ( $page ) {
			$page->add_field_element(
				array(
					...
					'description'   => esc_html__( 'Choose a default sidebar layout for pages', 'valeska-core' ),
			...
			if ( ! empty( $custom_sidebars ) && count( $custom_sidebars ) > 1 ) {
				$page->add_field_element(
					array(
						...
						'description' => esc_html__( 'Choose a custom sidebar to display on pages', 'valeska-core' ),
			...
			do_action( 'valeska_core_action_after_page_sidebar_options_map', $page );
		}
	}

	add_action( 'valeska_core_action_default_options_init', 'valeska_core_add_page_sidebar_options', valeska_core_get_admin_options_map_position( 'sidebar' ) );
}
