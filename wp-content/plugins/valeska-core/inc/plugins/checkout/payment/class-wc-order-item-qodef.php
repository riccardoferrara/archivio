	...
	*/
	...
	public function set_tax_class( $value ) {
		if ( $value && ! in_array( $value, WC_Tax::get_tax_class_slugs(), true ) ) {
			$this->error( 'order_item_product_invalid_tax_class', esc_html__( 'Invalid tax class', 'valeska-core' ) );
		}
	...
	public function set_product_id( $value ) {
		if ( $value > 0 && get_post_type( absint( $value ) ) !== $this->custom_post_type ) {
			$this->error( 'order_item_product_invalid_product_id', esc_html__( 'Invalid product ID', 'valeska-core' ) );
		}
	...
	public function set_variation_id( $value ) {
		if ( $value > 0 && 'product_variation' !== get_post_type( $value ) ) {
			$this->error( 'order_item_product_invalid_variation_id', esc_html__( 'Invalid variation ID', 'valeska-core' ) );
		}
	...
	public function set_product( $product ) {
		if ( ! is_a( $product, 'WC_Order_Item_Qodef' ) ) {
			$this->error( 'order_item_product_invalid_product', esc_html__( 'Invalid product', 'valeska-core' ) );
		}
	...
	public function set_backorder_meta() {
		...
		if ( $product && $product->backorders_require_notification() && $product->is_on_backorder( $this->get_quantity() ) ) {
			$this->add_meta_data( apply_filters( 'woocommerce_backordered_item_meta_name', esc_html__( 'Backordered', 'valeska-core' ), $this ), $this->get_quantity() - max( 0, $product->get_stock_quantity() ), true );
		}
	}
