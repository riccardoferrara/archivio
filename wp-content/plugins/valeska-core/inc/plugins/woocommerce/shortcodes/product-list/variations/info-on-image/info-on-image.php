...
if ( ! function_exists( 'valeska_core_add_product_list_variation_info_on_image' ) ) {
	/**
	 ...
	 * @return array
	 */
	function valeska_core_add_product_list_variation_info_on_image( $variations ) {
		$variations['info-on-image'] = esc_html__( 'Info On Image', 'valeska-core' );

		return $variations;
	}

	add_filter( 'valeska_core_filter_product_list_layouts', 'valeska_core_add_product_list_variation_info_on_image' );
}

if ( ! function_exists( 'valeska_core_register_shop_list_info_on_image_actions' ) ) {
	/**
	 * Function that override product item layout for current variation type
	 */
	function valeska_core_register_shop_list_info_on_image_actions() {
		...
		remove_action( 'woocommerce_shop_loop_item_title', 'valeska_woo_shop_loop_item_title', 10 ); // permission 10 is default
		add_action( 'woocommerce_before_shop_loop_item_title', 'valeska_woo_shop_loop_item_title', 17 ); // permission 17 is set because valeska_add_product_list_item_additional_image_holder hook is added on 15
		...
		add_action( 'woocommerce_before_shop_loop_item_title', 'woocommerce_template_loop_price', 19 ); // permission 19 is set because valeska_woo_shop_loop_item_title hook is added on 17
	...
	add_action( 'valeska_core_action_shop_list_item_layout_info-on-image', 'valeska_core_register_shop_list_info_on_image_actions' );
}
