...
class ValeskaCore_Titles {
	...
	public function __construct() {
		...
		add_filter( 'valeska_filter_page_title_classes', array( $this, 'add_page_title_classes' ) );

		// Add title area inline styles
		add_filter( 'valeska_filter_add_inline_style', array( $this, 'add_inline_styles' ) );
	}

	/**
	 * @return ValeskaCore_Titles
	 */
	...
	function include_layouts() {
		...
		foreach ( glob( VALESKA_CORE_INC_PATH . '/title/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}

		foreach ( glob( VALESKA_CORE_INC_PATH . '/title/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
	}

	function set_variables() {
		$layout_meta = valeska_core_get_post_value_through_levels( 'qodef_title_layout' );
		$layouts     = apply_filters( 'valeska_core_filter_register_title_layouts', $layouts = array() );
	...
	function render_template() {
		...
		if ( ! empty( $title_object ) ) {
			$template_hook = $title_object->overriding_whole_title ? 'valeska_filter_title_template' : 'valeska_filter_title_content_template';
	...
	function add_page_title_classes( $classes ) {
		$layout             = valeska_core_get_post_value_through_levels( 'qodef_title_layout' );
		$text_alignment     = valeska_core_get_post_value_through_levels( 'qodef_page_title_text_alignment' );
		$vertical_alignment = valeska_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$image_params       = valeska_core_get_page_title_image_params();
	...
	function add_inline_styles( $style ) {
		...
		$height                   = valeska_core_get_post_value_through_levels( 'qodef_page_title_height' );
		$height_mobile            = valeska_core_get_post_value_through_levels( 'qodef_page_title_height_on_smaller_screens' );
		$title_padding            = apply_filters( 'valeska_core_filter_title_padding', 0 );
		$title_padding_mobile     = apply_filters( 'valeska_core_filter_title_padding_mobile', 0 );
		$title_vertical_alignment = valeska_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$background_color         = valeska_core_get_post_value_through_levels( 'qodef_page_title_background_color' );
		$image_params             = valeska_core_get_page_title_image_params();
		...
		$title_color = valeska_core_get_post_value_through_levels( 'qodef_page_title_color' );
...
ValeskaCore_Titles::get_instance();
