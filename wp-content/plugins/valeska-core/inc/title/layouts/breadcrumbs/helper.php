...
if ( ! function_exists( 'valeska_core_register_breadcrumbs_title_layout' ) ) {
	/**
	 ...
	 * @return array
	 */
	function valeska_core_register_breadcrumbs_title_layout( $layouts ) {
		$layouts['breadcrumbs'] = 'ValeskaCore_Breadcrumbs_Title';

		return $layouts;
	}

	add_filter( 'valeska_core_filter_register_title_layouts', 'valeska_core_register_breadcrumbs_title_layout' );
}

if ( ! function_exists( 'valeska_core_add_breadcrumbs_title_layout_option' ) ) {
	/**
	 ...
	 * @return array
	 */
	function valeska_core_add_breadcrumbs_title_layout_option( $layouts ) {
		$layouts['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'valeska-core' );

		return $layouts;
	}

	add_filter( 'valeska_core_filter_title_layout_options', 'valeska_core_add_breadcrumbs_title_layout_option' );
}

if ( ! function_exists( 'valeska_core_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs html
	 */
	function valeska_core_breadcrumbs() {
		$page_id = qode_framework_get_page_id();

		// Breadcrumbs label
		$labels = apply_filters(
			'valeska_core_filter_breadcrumbs_label',
			array(
				'home'        => esc_html__( 'Home', 'valeska-core' ),
				'tag'         => esc_html__( 'Posts tagged "%s"', 'valeska-core' ),
				'author'      => esc_html__( 'Posted by %s', 'valeska-core' ),
				'search'      => esc_html__( 'Search results for "%s"', 'valeska-core' ),
				'404'         => esc_html__( '404 - Page not found', 'valeska-core' ),
				'query_paged' => esc_html__( '(Page %s)', 'valeska-core' ),
			)
		);

		// Breadcrumbs variables
		$settings = apply_filters(
			'valeska_core_filter_breadcrumbs_settings',
			array(
		...
		$breadcrumbs_html = '';
		if ( ! empty( $wrap ) ) {
			$breadcrumbs_html = $settings['wrap_before'] . $wrap . apply_filters( 'valeska_core_filter_breadcrumbs_content', $wrap_child, $settings ) . $settings['wrap_after'];
		}

		echo apply_filters( 'valeska_core_filter_breadcrumbs_template', $breadcrumbs_html );
	}
}
