...
class ValeskaCore_Mobile_Headers {
	...
	public function __construct() {
		...
		add_filter( 'valeska_filter_add_inline_style', array( $this, 'set_menu_typography_styles' ) );
	}

	/**
	 * @return ValeskaCore_Mobile_Headers
	 */
	...
	function include_elements() {
		...
		foreach ( glob( VALESKA_CORE_INC_PATH . '/mobile-header/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}
		foreach ( glob( VALESKA_CORE_INC_PATH . '/mobile-header/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
	}

	function set_variables() {
		$layout_meta = valeska_core_get_post_value_through_levels( 'qodef_mobile_header_layout' );
		$layouts     = apply_filters( 'valeska_core_filter_register_mobile_header_layouts', $header_layouts_option = array() );
	...
	function render_template() {
		...
		if ( ! empty( $header_object ) ) {
			$template_hook = $header_object->is_whole_header_override() ? 'valeska_filter_mobile_header_template' : 'valeska_filter_mobile_header_content_template';
	...
	function add_body_classes( $classes ) {
		$header_layout = valeska_core_get_post_value_through_levels( 'qodef_mobile_header_layout' );
		$classes[]     = ! empty( $header_layout ) ? 'qodef-mobile-header--' . $header_layout : '';

		$classes[] = 'yes' === valeska_core_get_post_value_through_levels( 'qodef_mobile_header_scroll_appearance' ) ? 'qodef-mobile-header-appearance--sticky' : '';
	...
	function add_header_widget_areas() {
		register_sidebar(
			array(
				...
				'name'          => esc_html__( 'Mobile Header', 'valeska-core' ),
				'before_widget' => '<div id="%1$s" class="widget %2$s qodef-mobile-header-widget-area-one" data-area="mobile-header">',
				'after_widget'  => '</div>',
				'description'   => esc_html__( 'Widgets added here will appear in mobile header widget area', 'valeska-core' ),
			)
		);
	}

	function set_menu_typography_styles( $style ) {
		$header_selector         = apply_filters( 'valeska_core_filter_nav_menu_mobile_header_selector', '.qodef-mobile-header-navigation' );
		$first_lvl_styles        = valeska_core_get_typography_styles( 'qodef_mobile_1st_lvl' );
		$first_lvl_hover_styles  = valeska_core_get_typography_hover_styles( 'qodef_mobile_1st_lvl' );
		$second_lvl_styles       = valeska_core_get_typography_styles( 'qodef_mobile_2nd_lvl' );
		$second_lvl_hover_styles = valeska_core_get_typography_hover_styles( 'qodef_mobile_2nd_lvl' );
		...
		$first_lvl_active_color = valeska_core_get_option_value( 'admin', 'qodef_mobile_1st_lvl_active_color' );
		...
		$second_lvl_active_color = valeska_core_get_option_value( 'admin', 'qodef_mobile_2nd_lvl_active_color' );
...
ValeskaCore_Mobile_Headers::get_instance();
