! function(y, c, i, n) {
    ...
    t.prototype.onReset = function(t) { t.preventDefault(), t.data.variationForm.$attributeFields.val("").trigger("change"), t.data.variationForm.$form.trigger("reset_data") }, t.prototype.onReload = function(t) {
        ...
        t = t.data.variationForm;
        t.$form.find('input[name="variation_id"], input.variation_id').val("").trigger("change"), t.$form.find(".wc-no-matching-variations").remove(), t.useAjax || t.$form.trigger("woocommerce_variation_select_change"), t.$form.trigger("woocommerce_variation_has_changed")
                        ...
                        if (v.hasOwnProperty(l)) {
                            ...
                            if (l === e)
                                if (m[d].variation_is_active && (u = "enabled"), g) {
                                    ...
                                    var f = n.find("option");
                                    if (f.length)
                                        for (var h = 0, p = f.length; h < p; h++) { var w = y(f[h]); if (g === w.val()) { w.addClass("attached " + u); break } }
                                } else n.find("option:gt(0)").addClass("attached " + u)
                        }
                }
            a = n.find("option.attached").length, s && (c = !1, 0 !== a && n.find("option.attached.enabled").each(function() { var t = y(this).val(); if (s === t) return !(c = !0) })), 0 < a && s && c && "no" === r && (n.find("option:first").remove(), o = ""), n.find("option" + o + ":not(.attached)").remove(), i.html(n.html()), i.find("option" + o + ":not(.enabled)").prop("disabled", !0), s ? c ? i.val(s) : i.val("").trigger("change") : i.val("")
        ...
        if (t && t.image && t.image.src && 1 < t.image.src.length) {
            ...
            if (0 < r.length) return r.trigger("click"), a.attr("current-image", t.image_id), void c.setTimeout(function() { y(c).trigger("resize"), e.trigger("woocommerce_gallery_init_zoom") }, 20);
        ...
        c.setTimeout(function() { y(c).trigger("resize"), a.wc_maybe_trigger_slide_position_reset(t), e.trigger("woocommerce_gallery_init_zoom") }, 20)
    ...
    }, y(function() { "undefined" != typeof wc_add_to_cart_variation_params && y(".variations_form").each(function() { y(this).wc_variation_form() }) });
    var _ = function(t) {
